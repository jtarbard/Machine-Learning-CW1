...
class MLP:
    ...
    def train(self, inputs, targets, eta, niterations):
        ...
        for n in range(niterations):
            ...
            deltao = ((self.outputs - targets) * self.outputs * (1.0 - self.outputs)) / ndata
            
            # compute the derivative of the second hidden layer
            deltah2 = self.hidden2 * self.beta * (1.0 - self.hidden2) * np.dot(deltao, self.weights3.T)
           
            # compute the derivative of the first hidden layer
            deltah1 = self.hidden1 * self.beta * (1.0 - self.hidden1) * np.dot(deltah2[:, :-1], self.weights2.T)
    ...
    def forwardPass(self, inputs):
        ...
        self.hidden1 = 1.0 / (1.0 + np.exp(-self.beta * self.hidden1)) #sigmoid activation
        ...
        self.hidden2 = 1.0 / (1.0 + np.exp(-self.beta * self.hidden2)) #sigmoid activation
